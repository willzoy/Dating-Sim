...
global.TGMS_HIDE = TweenFireFrom;
	...
	*/
	...
	if (_propCount == 1)
	{
	    ...
	    if (is_array(_propRaw)) // Advanced Property
	    {
	        ...
	        { // Normalized
	            _data[0] = _propRaw[1]; // data    
				_data[1] = argument[7]; // start
	            _data[2] = script_execute(global.__PropertyGetters__[? _propRaw[0]], _propRaw[1], _target); // destination
            
	            return TGMS_TweenFire(_target, _propRaw[0], argument[1], argument[2], argument[3], argument[4], argument[5], 0, 1, _data);
	        }
	        else
	        { // Not normalized
	            return TGMS_TweenFire(_target, _propRaw[0], argument[1], argument[2], argument[3], argument[4], argument[5], argument[7], script_execute(global.__PropertyGetters__[? _propRaw[0]], _propRaw[1], _target), _propRaw[1]);
	        }   
	    }
	    else // Standard Property
	    {
	        ...
	        if (ds_map_exists(global.__NormalizedProperties__, _propRaw)) // Normalized Property
	        {
	            ...
	            _data[1] = script_execute(global.__PropertyGetters__[? _propRaw], _target); // destination
	        ...
	        {
	            return TGMS_TweenFire(_target, _propRaw, argument[1], argument[2], argument[3], argument[4], argument[5], argument[7], script_execute(global.__PropertyGetters__[? _propRaw], _target));
	        }
			else // Dynamic Property
			{
	...
	repeat(_propCount)
	{
	    ...
	    if (is_array(_propRaw)) // Extended Properties
	    {
	        ...
	        {   // Standard Property =================
				_extData[++_extIndex] = argument[++_argIndex]; // start
				_extData[++_extIndex] = script_execute(global.__PropertyGetters__[? _propRaw[0]], _propRaw[1], _target); // dest
	            _extData[++_extIndex] = _propRaw[1]; // Property Data
	        }
	    ...
	    else // Standard Properties
	    {
			if (ds_map_exists(global.__PropertySetters__, _propRaw))
			{
		        ...
		        {    
		            ...
		            _data = 0;
					_data[0] = argument[++_argIndex]; // real start value
		            _data[1] = script_execute(global.__PropertyGetters__[? _propRaw], _target); // real destination value
		            _extData[++_extIndex] = 0; // normalized start
		            _extData[++_extIndex] = 1; // normalized destination
		            _extData[++_extIndex] = _data; // data
		        }
		        else
		        {
					// Standard Property
					_extData[++_extIndex] = argument[++_argIndex]; // start
		            _extData[++_extIndex] = script_execute(global.__PropertyGetters__[? _propRaw], _target); // destination
		            _extData[++_extIndex] = _target; // target
		        }
